...
export class LikesController {
  ...
  @ApiOperation({
    summary: '2.1.4, 2.2.3. - 게시글 좋아요 설정/해제 기능',
    description:
      '둘러보기 탐색/팔로잉에서 사용되는 API이다. 둘러보기 게시물들 중에 마음에드는 피드에 좋아요 버튼을 눌러 좋아요를 설정/해제 할 수 있다.\n\
                      좋아요 상태에서는 좋아요 해제 되고 좋아요를 안한 상태에서는 좋아요가 된다.',
  })
  @ApiResponse({
  ...
  @Post()
  postLikes(@Body() postLikeRequestDTO: PostLikeRequestDTO) {
    return this.likesService.postLikes(
      postLikeRequestDTO.feedId,
      postLikeRequestDTO.profileId,
    );
  }
}
